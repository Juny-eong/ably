package com.ably.assignment.global.encrypt;
...
@Component
public final class SEEDEncoder {

    private static String HexStringKey; // secret

    @Value("${encrypt.secret}")
    public void setHexStringKey(String key) {
        HexStringKey = key;
    }

    public static String encrypt(String text) {
        byte[] key = hexStringToByteArray(HexStringKey);
        byte[] textToByte = text.getBytes(StandardCharsets.UTF_8);
        return new String(  Base64.getEncoder().encode(  SEED_ECB_Encrypt(key, textToByte, 0, textToByte.length))  );
    }

    public static String decrypt(String text) {
        byte[] key = hexStringToByteArray(HexStringKey);
        byte[] textToByte = Base64.getDecoder().decode(text);
        return new String(    SEED_ECB_Decrypt(key, textToByte, 0, textToByte.length)  );
    }
